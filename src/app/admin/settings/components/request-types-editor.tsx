...
interface RequestTypesEditorProps {
  initialRequestTypes: CauRequestType[];
  categories: CauCategory[];
}

export default function RequestTypesEditor({ initialRequestTypes, categories }: RequestTypesEditorProps) {
  const [requestTypes, setRequestTypes] = useState<CauRequestType[]>(initialRequestTypes);
  ...
  const handleSaveType = async (requestType: CauRequestType) => {
    ...
    await writeData('cau-request-types.json', updatedRequestTypes);
    setEditingType(null);
  };
